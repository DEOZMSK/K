...
async function getQuestionsContent(): Promise<QuestionsContent> {
  ...
  let currentCategory: QuestionCategory["name"] | null = null;
  let buffer: string[] = [];
...
export default async function QuestionsPage() {
  const { introduction, categories } = await getQuestionsContent();
    ...
    <main className="relative min-h-screen overflow-hidden bg-gradient-to-br from-[#fdf6e8] via-[#f8e6c9] to-[#f3d9aa] text-neutral-900">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-5xl flex-col gap-10 px-6 py-14 sm:gap-14 sm:px-10 lg:px-12">
        ...
        {introduction.length > 0 && (
          <div className="space-y-3 rounded-3xl border border-[#cda15e]/25 bg-white/60 p-6 text-sm leading-relaxed text-neutral-700 shadow-[0_14px_34px_rgba(125,84,25,0.08)] sm:text-base">
